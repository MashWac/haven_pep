...
<main class="flex-1 flex flex-col h-full overflow-hidden bg-background-light dark:bg-background-dark relative">
    <header class="flex items-center justify-between whitespace-nowrap border-b border-[#e5e2dc] dark:border-gray-800 bg-white dark:bg-[#1f1a12] px-8 py-4 z-10">
        <div class="flex items-center gap-4 text-[#171511] dark:text-white">
            ...
            <h2 class="text-xl font-bold leading-tight tracking-tight">Sales Management</h2>
        </div>
    ...
    <div class="flex-1 overflow-y-auto p-8">
        <div class="max-w-7xl mx-auto flex flex-col gap-8">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a class="text-gray-500 hover:text-brand-orchid transition-colors text-sm font-medium" href="#">Sales</a>
                <span class="material-symbols-outlined text-gray-400 text-sm">chevron_right</span>
                <span class="text-[#171511] dark:text-white text-sm font-bold bg-primary/30 dark:bg-primary/10 px-2 py-0.5 rounded text-brand-orchid">All Sales</span>
            </div>
            <!-- Success Message -->
            @if(session('success'))
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-[#171511] dark:text-white text-3xl font-black leading-tight tracking-tight">All Sales</h1>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr class="group hover:bg-primary/5 dark:hover:bg-white/5 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div>
                                            <p class="font-bold text-[#171511] dark:text-white text-sm group-hover:text-brand-orchid transition-colors">{{ $sale->transaction_reference }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <span class="text-gray-700 dark:text-gray-300">{{ $sale->created_at }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{{ $sale->buyer_name }}</td>
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{{ $sale->email_address }}</td>

                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{url('/admin_sales/details/'. $sale->id)}}" class="p-1.5 text-gray-400 hover:text-brand-orchid hover:bg-brand-orchid/10 rounded-md transition-colors" title="Edit">
