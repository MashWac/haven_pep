...
<main class="flex-1 flex flex-col h-full relative overflow-y-auto bg-background-light dark:bg-background-dark">
    <div class="layout-container flex flex-col w-full max-w-[900px] mx-auto px-6 py-8">
        ...
        <div class="flex flex-wrap gap-2 mb-6 items-center">
            ...
            <span class="material-symbols-outlined text-[#877b64] text-sm">chevron_right</span>
            <span class="text-[#171511] dark:text-white text-sm font-medium">Edit Item</span>
        </div>
        ...
        <form action="{{ url('/admin_shop/update/'.$data['item']->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            {{-- Sticky header --}}
            <div class="flex flex-wrap justify-between items-end gap-4 mb-8 sticky top-0 bg-background-light dark:bg-background-dark z-20 py-2 border-b border-transparent">
                <div class="flex flex-col gap-2">
                    <h1 class="text-[#171511] dark:text-white text-3xl font-black">Edit Shop Item</h1>
                    <p class="text-[#877b64] text-base">Update the details for "{{ $data['item']->name }}".</p>
                </div>
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="flex items-center justify-center px-4 h-10 rounded-lg bg-[#DA70D6] text-white text-sm font-bold shadow-md hover:opacity-90">
                        <span class="material-symbols-outlined text-[20px] mr-2">save</span>Update Item
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
                    ...
                    <section class="bg-white dark:bg-[#2c261a] rounded-xl p-6 shadow-sm border border-[#e5e2dc]/50 dark:border-[#362f22]">
                        ...
                        <div class="flex flex-col gap-4">
                            <label class="flex flex-col gap-2">
                                ...
                                <input name="name" type="text" required value="{{ old('name', $data['item']->name) }}"
                            ...
                            <label class="flex flex-col gap-2">
                                ...
                                <select name="category_id" required
                                    ...
                                    <option value="{{ $category->id }}" {{ old('category_id', $data['item']->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </label>
                        </div>
                        ...
                        <label class="flex flex-col gap-2">
                            ...
                            <div class="w-full rounded-lg border border-[#e5e2dc] dark:border-[#4a402e] overflow-hidden focus-within:ring-2 focus-within:ring-[#DA70D6]/20 focus-within:border-[#DA70D6] transition-all">
                                <textarea name="description" rows="5"
                                    ...
                                    placeholder="Write a detailed description...">{{ old('description', $data['item']->description) }}</textarea>
                            </div>
                        </label>
                    </section>
                </div>
                ...
                <div class="flex flex-col gap-6">
                    <section class="bg-white dark:bg-[#2c261a] rounded-xl p-6 shadow-sm border border-[#e5e2dc]/50 dark:border-[#362f22]">
                        ...
                        @if($data['item']->image)
                        <div class="mb-3">
                            <img src="{{ asset($data['item']->image) }}" alt="{{ $data['item']->name }}" class="w-full h-40 object-cover rounded-lg border border-[#e5e2dc] dark:border-[#4a402e]">
                            <p class="text-xs text-[#877b64] text-center mt-1">Current image — upload a new one to replace.</p>
                        </div>
                        @endif
                        <div id="image-preview-wrapper" class="{{ $data['item']->image ? 'hidden' : 'hidden' }}">
                            <img id="image-preview" src="" alt="New Preview" class="w-full h-40 object-cover rounded-lg mb-2">
                        </div>
                        <label for="item-image" class="flex flex-col items-center justify-center w-full h-36 border-2 border-[#e5e2dc] border-dashed rounded-lg cursor-pointer bg-background-light dark:bg-[#362f22] dark:border-[#4a402e] hover:bg-primary/30 transition-colors group">
                    ...
                    <section class="bg-white dark:bg-[#2c261a] rounded-xl p-6 shadow-sm border border-[#e5e2dc]/50 dark:border-[#362f22]">
                        ...
                        <div class="flex flex-col gap-4">
                            <label class="flex flex-col gap-2">
                                ...
                                <input name="price" type="number" step="0.01" min="0" required value="{{ old('price', $data['item']->price) }}"
                            ...
                            <label class="flex flex-col gap-2">
                                ...
                                <input name="discount_percentage" type="number" min="0" max="100" value="{{ old('discount_percentage', $data['item']->discount_percentage) }}"
                                    class="form-input w-full rounded-lg border border-[#e5e2dc] bg-background-light dark:bg-[#362f22] dark:border-[#4a402e] dark:text-white h-11 px-4 text-sm focus:ring-2 focus:ring-[#DA70D6]/20 focus:border-[#DA70D6] outline-none transition-all">
                            </label>
                        </div>
                    </section>
