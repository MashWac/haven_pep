...
<main class="flex-1 flex flex-col h-full relative overflow-y-auto bg-background-light dark:bg-background-dark">
    <div class="layout-container flex flex-col w-full max-w-[900px] mx-auto px-6 py-8">
        ...
        <div class="flex flex-wrap gap-2 mb-6 items-center">
            ...
            <span class="material-symbols-outlined text-[#877b64] text-sm">chevron_right</span>
            <span class="text-[#171511] dark:text-white text-sm font-medium">Create Combo</span>
        </div>
        ...
        <form action="{{ url('/admin_shop_combos/insert') }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{-- Sticky Header --}}
            <div class="flex flex-wrap justify-between items-end gap-4 mb-8 sticky top-0 bg-background-light dark:bg-background-dark z-20 py-2 border-b border-transparent">
                <div class="flex flex-col gap-2">
                    <h1 class="text-[#171511] dark:text-white text-3xl font-black">Create New Combo</h1>
                    <p class="text-[#877b64] text-base">Bundle shop items together into a discounted deal.</p>
                </div>
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="flex items-center justify-center px-4 h-10 rounded-lg bg-[#DA70D6] text-white text-sm font-bold shadow-md hover:opacity-90">
                        <span class="material-symbols-outlined text-[20px] mr-2">save</span>Save Combo
                    </button>
                </div>
            </div>
                ...
                <div class="lg:col-span-2 flex flex-col gap-6">
                        ...
                        <div class="flex flex-col gap-4">
                            <label class="flex flex-col gap-2">
                                ...
                                <input name="name" type="text" required value="{{ old('name') }}"
                                    ...
                                    placeholder="e.g. Starter Wellness Pack">
                            </label>
                            <label class="flex flex-col gap-2">
                                ...
                                <div class="w-full rounded-lg border border-[#e5e2dc] dark:border-[#4a402e] overflow-hidden focus-within:ring-2 focus-within:ring-[#DA70D6]/20 transition-all">
                                    <textarea name="description" rows="4"
                                        ...
                                        placeholder="Describe this combo deal...">{{ old('description') }}</textarea>
                                </div>
                            </label>
                        </div>
                    </section>
                    ...
                    <section class="bg-white dark:bg-[#2c261a] rounded-xl p-6 shadow-sm border border-[#e5e2dc]/50 dark:border-[#362f22]">
                        ...
                        @if($data['items']->isEmpty())
                            <div class="text-sm text-gray-400 flex items-center gap-2 py-4">
                                <span class="material-symbols-outlined">warning</span>
                                No shop items found. <a href="{{ url('/admin_shop/add') }}" class="text-brand-orchid hover:underline ml-1">Create some shop items first.</a>
                            </div>
                        @else
                        {{-- Search filter --}}
                        ...
                        <div class="flex flex-col gap-2 max-h-64 overflow-y-auto pr-1 item-list">
                            ...
                            <label id="item-row-{{ $item->id }}" class="flex items-center gap-3 p-3 rounded-lg border border-[#e5e2dc] dark:border-[#4a402e] cursor-pointer hover:bg-[#DA70D6]/5 transition-colors has-[:checked]:border-[#DA70D6] has-[:checked]:bg-[#DA70D6]/5">
                                <input type="checkbox" name="items_included[]" value="{{ $item->id }}"
                                    {{ is_array(old('items_included')) && in_array($item->id, old('items_included')) ? 'checked' : '' }}
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <section class="bg-white dark:bg-[#2c261a] rounded-xl p-6 shadow-sm border border-[#e5e2dc]/50 dark:border-[#362f22]">
                        ...
                        <div class="flex flex-col items-center justify-center w-full">
                            ...
                            <label for="combo-image" class="flex flex-col items-center justify-center w-full h-44 border-2 border-[#e5e2dc] border-dashed rounded-lg cursor-pointer bg-background-light dark:bg-[#362f22] dark:border-[#4a402e] hover:bg-primary/30 transition-colors group">
                                <span class="material-symbols-outlined text-4xl text-[#DA70D6] mb-2 group-hover:scale-110 transition-transform">upload</span>
                                <p class="text-sm text-[#171511] dark:text-white"><span class="font-bold">Click to upload</span></p>
                                <p class="text-xs text-[#877b64] mt-1">PNG, JPG (MAX. 2MB)</p>
