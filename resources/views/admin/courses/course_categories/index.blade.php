...
<main class="flex-1 flex flex-col h-full overflow-hidden bg-background-light dark:bg-background-dark relative">
    <header class="flex items-center justify-between whitespace-nowrap border-b border-[#e5e2dc] dark:border-gray-800 bg-white dark:bg-[#1f1a12] px-8 py-4 z-10">
        <div class="flex items-center gap-4 text-[#171511] dark:text-white">
            ...
            <h2 class="text-xl font-bold leading-tight tracking-tight">Course Category Management</h2>
        </div>
    ...
    <div class="flex-1 overflow-y-auto p-8">
        <div class="max-w-7xl mx-auto flex flex-col gap-8">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a class="text-gray-500 hover:text-brand-orchid transition-colors text-sm font-medium" href="{{ url('/admin_courses') }}">Courses</a>
                <span class="material-symbols-outlined text-gray-400 text-sm">chevron_right</span>
                <span class="text-[#171511] dark:text-white text-sm font-bold bg-primary/30 dark:bg-primary/10 px-2 py-0.5 rounded text-brand-orchid">All Course Categories</span>
            </div>
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <h1 class="text-[#171511] dark:text-white text-3xl font-black leading-tight tracking-tight">All Course Categories</h1>
                    <p class="text-gray-500 dark:text-gray-400 text-base">Manage your course categories, track enrollments, and update content.</p>
                </div>
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <a href="{{ url('/add_course_category') }}" class="flex items-center justify-center px-4 h-10 rounded-lg bg-[#DA70D6] text-white text-sm font-bold shadow-md hover:opacity-90 transition-opacity">
                            ...
                            <tr class="group hover:bg-primary/5 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{url('/admin_course_categories/edit/'. $category->id)}}" class="p-1.5 text-gray-400 hover:text-brand-orchid hover:bg-brand-orchid/10 rounded-md transition-colors" title="Edit">
                                            <span class="material-symbols-outlined text-lg">edit</span>
                                        </a>
                                        <form action="{{ url('/admin_course_categories/delete/' . $category->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
