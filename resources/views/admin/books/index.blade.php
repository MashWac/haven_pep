...
<main class="flex-1 flex flex-col h-full overflow-hidden bg-background-light dark:bg-background-dark relative">
    <header class="flex items-center justify-between whitespace-nowrap border-b border-[#e5e2dc] dark:border-gray-800 bg-white dark:bg-[#1f1a12] px-8 py-4 z-10">
        <div class="flex items-center gap-4 text-[#171511] dark:text-white">
            ...
            <h2 class="text-xl font-bold leading-tight tracking-tight">Books Management</h2>
        </div>
    ...
    <div class="flex-1 overflow-y-auto p-8">
        <div class="max-w-7xl mx-auto flex flex-col gap-8">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a class="text-gray-500 hover:text-brand-orchid transition-colors text-sm font-medium" href="#">Books</a>
                <span class="material-symbols-outlined text-gray-400 text-sm">chevron_right</span>
                <span class="text-[#171511] dark:text-white text-sm font-bold bg-primary/30 dark:bg-primary/10 px-2 py-0.5 rounded text-brand-orchid">All Books</span>
            </div>
                    <!-- Success Message -->
        @if(session('success'))
        ...
        @endif
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <a href="{{ url('admin_book_categories') }}" class="flex items-center justify-center px-4 h-10 rounded-lg border border-[#e5e2dc] bg-white dark:bg-[#2c261a] dark:border-[#362f22] dark:text-white text-[#171511] text-sm font-medium hover:bg-gray-50 dark:hover:bg-[#362f22]/80 transition-colors">
                        Create Book Category
                    </a>
                    <a href="{{ url('/admin_add_book') }}" class="flex items-center justify-center px-4 h-10 rounded-lg bg-[#DA70D6] text-white text-sm font-bold shadow-md hover:opacity-90 transition-opacity">
                    ...
                    <table class="w-full text-left text-sm adminTables">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr class="group hover:bg-primary/5 dark:hover:bg-white/5 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div class="w-16 h-10 rounded bg-gray-200 bg-cover bg-center shrink-0" style="background-image: url('{{ asset($book->image) }}');"></div>
                                        <div>
                                            <p class="font-bold text-[#171511] dark:text-white text-sm group-hover:text-brand-orchid transition-colors">{{ $book->title }}</p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">ID: #{{ $book->isbn }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">${{ $book->pricing }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{url('/admin_book/edit/'. $book->id)}}" class="p-1.5 text-gray-400 hover:text-brand-orchid hover:bg-brand-orchid/10 rounded-md transition-colors" title="Edit">
                                            <span class="material-symbols-outlined text-lg">edit</span>
                                        </a>
                                        <form action="{{ url('/admin_book/delete/' . $book->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
