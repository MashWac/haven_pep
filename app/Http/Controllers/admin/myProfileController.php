...
class myProfileController extends Controller
{
    public function index()
    {
        ...
        $data['bubbles'] = AboutMeBubbleModel::orderBy('rank_position', 'asc')->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'bubbles' => 'nullable|array|max:5',
            'bubbles.*.text' => 'required_with:bubbles|string|max:255',
            'bubbles.*.rank_position' => 'required_with:bubbles|integer',
        ]);
        ...
        // Handle Bubbles
        if ($request->has('bubbles')) {
            AboutMeBubbleModel::truncate(); // Global if no user_id, or just clear existing
            AboutMeBubbleModel::insert(
                collect($request->bubbles)
